...
class CNNProcessor(DataProcessor):
  ...
  def get_test_examples(self, data_dir, hier, tax):
    ...
    file_name = hier + "_" + tax + "_"
    return self._create_examples(
        self._read_tsv(os.path.join(data_dir, file_name+"test_5.tsv")), "test")
...
def get_batches_elmo_multi_3(train_examples, embed_list, batch_size=64, random_flag=True):
    samples = train_examples
    num_samples = len(samples)
    if random_flag:
        shuffle(samples)
...
def get_batches_elmo_multi_4(train_examples, embed_list, batch_size=64, random_flag=True):
    samples = train_examples
    num_samples = len(samples)
    if random_flag:
        shuffle(samples)
...
def get_vector_from_elmo(embed, key, length=1024):
    if key in embed:
        result = embed[key]
...
def get_multi_embeddings_from_elmo(examples, embedding_type_list, batch_size=500):
    ...
    if "doc2vec_pvdbow" in embedding_type_list:
        ...
        #  PV-DBOW
        vector_model_file_0 = FLAGS.vector_model_path + "concept_model0"
    ...
    if "doc2vec_pvdm" in embedding_type_list:
        ...
        tokenizer = tokenization.FullTokenizer(vocab_file=FLAGS.vocab_file, do_lower_case=FLAGS.do_lower_case)
        # PV-DM seems better??
        vector_model_file_1 = FLAGS.vector_model_path + "concept_model1"
...
def main(_):
    ...
    if FLAGS.do_train:
        ...
        for i, model in enumerate(models):
            ...
            history = model.fit_generator(generator=get_batches_elmo_multi_3(train_examples, embed_list=embeddings_list, batch_size=FLAGS.batch_size),
                                          steps_per_epoch=len(train_examples) // FLAGS.batch_size,
                                          validation_data=get_batches_elmo_multi_3(validation_examples, embed_list=embeddings_list, batch_size=FLAGS.batch_size),
    ...
    if FLAGS.do_eval:
        ...
        eval_history = model.evaluate_generator(
            get_batches_elmo_multi_3(eval_examples, embed_list=embeddings_list, batch_size=FLAGS.batch_size),
    ...
    if FLAGS.do_predict:
        ...
        true_labels = []
        i = 0
        for x, y in get_batches_elmo_multi_4(predict_examples, embed_list=embeddings_list, batch_size=FLAGS.batch_size, random_flag=True):
...
if __name__ == "__main__":
    ...
    FLAGS.output_dir = FLAGS.directory_path +"cnnModel/" + FLAGS.hier_name + "/multi_models/Iteration_12/" + FLAGS.taxonomy + "/"
    FLAGS.vector_model_path = FLAGS.directory_path + "vectorModel/" + FLAGS.hier_name + "/" + str(FLAGS.iterations) + "/"
    FLAGS.saved_model_name = "concept_100_8_model"
