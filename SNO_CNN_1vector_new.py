...
batch_size = 6000  # Batch size : 6000  debug ****
seq_len = 512  # word embedding length
learning_rate = 0.0001
lambda_loss_amount = 0.001
epochs = 20000  # 20000  debug ****
n_classes = 2
if one_vector_flag:
    n_channels = 2
else:
...
# for iterations in iterations_list:
    ...
    # paired_list, not_paired_list = readFromJsonData2(test_data)
    ...
    # remove duplicates
    not_paired_list = remove_duplicates(not_paired_list)
    ...
    with tf.Session(graph=graph) as sess:
        # Restore
        saver.restore(sess, tf.train.latest_checkpoint(cnn_model_path))
        multiparents_concept_counter = 0
        for key, parents in parents_dict.items():
            print("Processing key ", key)
            if len(parents) > 1:
                multiparents_concept_counter += 1
                for i in range(len(parents)):
                    ...
                    if pair_list:
                        ...
                        for x_t, y_t in get_batches(idpairs_list, label_list, one_vector_flag=one_vector_flag,
                                                    ...
                                                    pvdm_model=pvdm_model, batch_size=len(idpairs_list),
                                                    random_flag=False, op_str=parent_str):
                            feed = {inputs_: x_t,
                                    labels_: y_t,
                                    keep_prob_: 1}
    ...
    print("Test accuracy: {:.6f}".format(np.mean(test_rest_acc)))
    print("Multiple parents concept counts: {}".format(multiparents_concept_counter))
