...
iterations_list = [10, 10, 10, 10, 10, 10, 10, 10, 10, 10]
# iterations_list = [20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50]   # debug ****
hier_list = ["clinical_finding"]  # "procedure"
...
# for iterations in iterations_list:
    ...
    if negative_flag == "hier":
        cnn_model_path = directory_path + "cnnModel/" + hier_name + "/hier/" + vector_type + "/" + str(iterations) + "/"
        ...
        cnn_model_path = directory_path + "cnnModel/" + hier_name + "/area/" + vector_type + "/" + str(iterations) + "/"
        notPair_file = data_path + "taxNotPairs_sno_" + hier_name + "_area.txt"
        img_name = hier_name + "_area_" + vector_type + "_iter_" + str(iterations) + "_"
    else:
        cnn_model_path = directory_path + "cnnModel/" + hier_name + "/parea/" + vector_type + "/" + str(iterations) + "/"
    ...
    test_missingisa = data_path + "missingisa_ids.txt"
    missingisa_list, _ = read_pair(test_missingisa)

    neg_missingisa_list = []
    for item in missingisa_list:
        neg_missingisa_list.append([item[1], item[0], 0])

    missingisa_ids_list = []
    for item in missingisa_list:
        missingisa_ids_list.append([item[0], item[1]])
    neg_missingisa_ids_list = []
    for item in neg_missingisa_list:
        neg_missingisa_ids_list.append([item[0], item[1]])

    idpairs_list, label_list = remove_testing_samples_from_training(missingisa_ids_list, idpairs_list, label_list)
    idpairs_list, label_list = remove_testing_samples_from_training(neg_missingisa_ids_list, idpairs_list, label_list)
    ...
    # test new data
    print('\n\nTesting missing IS-As new data')

    # remove duplicates
    missingisa_list = remove_duplicates(missingisa_list)
    print("remove duplicates in positive test data, the number is: {}".format(len(missingisa_list)))
    # remove duplicates
    neg_missingisa_list = remove_duplicates(neg_missingisa_list)
    print("remove duplicates in negative test data, the number is: {}".format(len(neg_missingisa_list)))
    ...
    data_list, label_list = readFromPairList(missingisa_list, neg_missingisa_list)
