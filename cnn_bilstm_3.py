...
def get_multi_embeddings_from_elmo(examples, embedding_type_list, batch_size=500):
    # Create graph and finalize (optional but recommended).
    concept_dict = {}
    ...
    if "elmo" in embedding_type_list:
        print("get embeddings from source: elmo")
    ...
    if "doc2vec_pvdbow" in embedding_type_list:
        ...
        #  PV-DBOW
        vector_model_file_0 = FLAGS.vector_model_path + "procedure_512_nondefault_whole_model0"
    ...
    if "doc2vec_pvdm" in embedding_type_list:
        ...
        tokenizer = tokenization.FullTokenizer(vocab_file=FLAGS.vocab_file, do_lower_case=FLAGS.do_lower_case)
        # PV-DM seems better??
        vector_model_file_1 = FLAGS.vector_model_path + "procedure_512_nondefault_whole_model1"
...
flags.DEFINE_string("output_predict_file", "output_predict.txt", "save output predict result to a file")
...
def main(_):
    ...
    embeddings_list = get_multi_embeddings_from_elmo(examples, embedding_type_list=embedding_types, batch_size=FLAGS.embed_batch_size)

    print("load embeddings done")
    one_value =next(iter(embeddings_list[1].values()))
    ...
    if FLAGS.do_train:
        ...
        model0 = Sequential([
            Conv1D(filters=256, kernel_size=15, strides=1, padding='same', activation='relu',
                   input_shape=(FLAGS.seq_len, FLAGS.n_channels), kernel_regularizer=l2(0.01)),
            MaxPooling1D(),
        ...
        mcp_save = ModelCheckpoint(FLAGS.output_dir + 'model-{epoch:03d}-{acc:03f}-{val_acc:03f}.hdf5', save_best_only=True,
    ...
    if FLAGS.do_eval:
        ...
        model = load_model(FLAGS.output_dir + FLAGS.saved_model_name + FLAGS.saved_model_index + ".hdf5")
    ...
    if FLAGS.do_predict:
        ...
        from keras.models import load_model
        files = []
        for i in os.listdir(FLAGS.output_dir):
            if os.path.isfile(os.path.join(FLAGS.output_dir, i)) and (FLAGS.saved_model_name + FLAGS.saved_model_index) in i:
                files.append(i)
        print('loading model file name: ',files[0])
        model = load_model(os.path.join(FLAGS.output_dir, files[0]))
        ...
        true_labels = []
        i = 0
        for x, y in get_batches_one_iter(predict_examples, embed_list=embeddings_list, batch_size=FLAGS.batch_size, random_flag=False):
            result = model.predict(x)
            predict_prob.extend(result)
            i += len(y)
            if i % FLAGS.batch_size == 0:
                print("Predicting progress: {:.2f}%".format(i/len(predict_examples)*100))
        ...
        plt.show()
        plot_confusion_matrix(true_labels, predict_labels, img_path=FLAGS.img_path, img_name=FLAGS.img_name + "predict_")

        with tf.gfile.GFile(FLAGS.output_predict_file, "w") as writer:
            num_written_lines = 0
            tf.logging.info("***** Predict results *****")

            from sklearn.metrics.pairwise import cosine_similarity
            #  PV-DBOW
            vector_model_file_0 = FLAGS.vector_model_path + FLAGS.hier_name + "_512_nondefault_whole_model0"
            pvdbow_model = gensim.models.Doc2Vec.load(vector_model_file_0)
            tokenizer = tokenization.FullTokenizer(vocab_file=FLAGS.vocab_file, do_lower_case=FLAGS.do_lower_case)

            def cal_sim_score(vector_model, example, tokenizer):
                if example.id_a in vector_model.docvecs:
                    concept_vector = vector_model.docvecs[example.id_a]
                else:
                    concept_vector = vector_model.infer_vector(tokenizer.tokenize(example.text_a))

                if example.id_b in vector_model.docvecs:
                    sim_vector = vector_model.docvecs[example.id_b]
                else:
                    sim_vector = vector_model.infer_vector(tokenizer.tokenize(example.text_b))
                sim_score = cosine_similarity([concept_vector], [sim_vector])
                return sim_score[0]

            output_result = []
            for example, prob in zip(predict_examples, predict_prob):
                sim_score = cal_sim_score(pvdbow_model, example, tokenizer)
                output_result.append([example.id_a, example.id_b, example.text_a, example.text_b, example.label, prob, sim_score])

            text_line = "\t".join(str(token) for token in ['Child', 'Parent', 'Child_label', 'Parent_label', 'True label', 'IS-A Prob', 'Similarity Score']) + "\n"
            writer.write(text_line)
            for result in output_result:
                text_line = "\t".join(str(token) for token in result) + "\n"
                writer.write(text_line)
...
if __name__ == "__main__":
    ...
    FLAGS.embed_batch_size = 300
    FLAGS.epochs = 20  # 10
    # FLAGS.data_dir = "../bert/glue_data/MYTESTING/Iteration_9/"  # kong
    FLAGS.data_dir = "SNO/TestData/Iteration_9/"  # local
    FLAGS.learning_rate = 0.0001
    FLAGS.output_dir = FLAGS.directory_path +"cnnModel/" + FLAGS.hier_name + "/multi_models/2docvec_whole/" + FLAGS.taxonomy + "/"
    FLAGS.vector_model_path = FLAGS.directory_path + "vectorModel/" + FLAGS.hier_name + "/" + str(FLAGS.iterations) + "/"
    FLAGS.saved_model_name = "model"
    FLAGS.saved_model_index = "0"
    FLAGS.output_predict_file = "output_predict_hier.txt"
